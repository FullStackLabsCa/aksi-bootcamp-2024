...
public class HibernateUtilsTest {
    ...
    public void commitTransactionTableSizeTest() {
        ...
        String hql = "Select count(p) from RawPayload p";
    ...
    public void commitTransactionTableDataTest() {
        ...
        String hql = "from RawPayload";
        Query query = session.createQuery(hql, RawPayload.class);
        List<RawPayload> rawPayloadBeforeCommitting = query.getResultList();

        assertTrue(rawPayloadBeforeCommitting.isEmpty());
        assertTrue(HibernateUtils.getInstance().getConnection().getTransaction().isActive());
        ...
        Query queryWithNewSession = HibernateUtils.getInstance().getConnection().createQuery(hql, Long.class);
        List<RawPayload> rawPayloadsAfterCommitting = queryWithNewSession.getResultList();

        assertEquals(1, rawPayloadsAfterCommitting.size());
        assertEquals(rawPayloadsAfterCommitting.get(0), rawPayload);
    ...
    public void rollbackTransactionTableDataTest() {
        ...
        assertEquals(0, rawPayloadsAfterCommitting.size());
        assertFalse(session.isOpen());
    }
}
